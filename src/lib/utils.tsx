...
import { CheckCircle2, Shield, Zap, Gem, Flame, Sparkles, Ghost, Cpu } from "lucide-react";
...
export const getBadgeIcon = (badge?: string, className?: string) => {
  const baseClass = cn("w-4 h-4 inline-block ml-1", className);
  switch (badge) {
    case 'verified-pro':
      return <CheckCircle2 className={cn(baseClass, "text-blue-500 fill-blue-500/20")} />;
    case 'owner':
      return <Shield className={cn(baseClass, "text-yellow-500 fill-yellow-500/20")} />;
    case 'ultra-verified':
      return <Zap className={cn(baseClass, "text-cyan-400 fill-cyan-400/20")} />;
    case 'crystal':
      return <Gem className={cn(baseClass, "text-indigo-400 fill-indigo-400/20")} />;
    case 'magma':
      return <Flame className={cn(baseClass, "text-orange-500 fill-orange-500/20")} />;
    case 'holographic':
      return <Sparkles className={cn(baseClass, "text-pink-400 fill-pink-400/20")} />;
    case 'steampunk':
      return <Cpu className={cn(baseClass, "text-amber-600 fill-amber-600/20")} />;
    case 'phantom':
      return <Ghost className={cn(baseClass, "text-slate-400 fill-slate-400/20")} />;
    default:
      return null;
  }
};
